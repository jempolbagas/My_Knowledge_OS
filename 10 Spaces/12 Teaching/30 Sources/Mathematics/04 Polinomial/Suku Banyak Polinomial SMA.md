---
title: "Master Dashboard: Suku Banyak (Polinomial)"
type: master-dashboard
subject: Mathematics
level: sma
target_audience: "SMA Kelas 11"
created: 2026-08-20
sources:
  - "[[Mempelajari Konsep dan Nilai Suku Banyak (Polinomial)]]"
tags:
  - teaching-material
  - mathematics
  - suku-banyak
  - polinomial
  - dashboard
---

# Master Dashboard: Menaklukkan Suku Banyak (Polinomial) 🚀

Selamat datang di pusat komando materi **Polinomial (Suku Banyak)**! Topik ini seringkali menjadi momok karena melibatkan pangkat tinggi dan rumus yang tampak rumit. Namun dengan pengelompokan modul secara sistematis, kamu akan bisa membedah dan menyelesaikan soal-soal HOTS dengan sangat mudah.

---

## 📚 Navigasi Modul Ajar

Silakan pelajari sub-modul berikut secara berurutan untuk pemahaman yang maksimal:

| Urutan | Modul Topik | Deskripsi Singkat |
| :---: | :--- | :--- |
| **1** | [[Konsep_dan_Kesamaan_Polinomial_SMA\|Konsep & Kesamaan Polinomial]] | Definisi, anatomi, syarat polinomial, dan trik pecahan parsial (Kesamaan). |
| **2** | [[Metode_Horner_dan_Operasi_SMA\|Metode Horner & Operasi]] | Cara sakti Horner, Horner pembagi linear, dan Horner-Kino pembagi kuadrat. |
| **3** | [[Teorema_Sisa_dan_Faktor_SMA\|Teorema Sisa & Faktor]] | Menebak sisa tanpa membagi penuh dan syarat penentu faktor suatu fungsi. |
| **4** | [[Pemfaktoran_dan_Akar_Rasional_SMA\|Pemfaktoran & Akar Rasional]] | (*HOTS*) Langkah sistematis memfaktorkan polinomial pangkat tinggi ($x^3, x^4$). |
| **5** | [[Teorema_Vieta_Polinomial_SMA\|Teorema Vieta Polinomial]] | (*HOTS*) Rumus rahasia jumlah dan hasil kali akar tanpa mencari akarnya. |

---

## 📝 Lembar Kerja & Evaluasi (Distingsi Level Kesulitan)

Uji pemahaman murid secara bertahap sesuai tingkat kemampuannya:
* **Lembar Praktik Kelompok:** [[LKPD Suku Banyak Polinomial SMA]]
* **🟢 Level 1 (Fondasi & Remidi):** [[Soal Suku Banyak SMA Level 1 Fondasi]] *(Fokus penguatan rumus, aljabar dasar, & LOTS-MOTS)*
* **🔵 Level 2 (Standar & PSTS/PAS):** [[Soal Suku Banyak SMA Level 2 Standar HOTS]] *(Fokus analisis aljabar, Horner-Kino, & HOTS Medium)*
* **📄 Paket Variasi Tambahan:** [[Soal Suku Banyak SMA Paket 1]] | [[Soal Suku Banyak SMA Paket 2]]

---

## ⚡ Cheatsheet Cepat Polinomial

Simpan ringkasan ini sebagai referensi cepat saat mengerjakan soal ujian!

**1. Penyesuaian Hasil Bagi Horner:**
Jika pembagi adalah $(ax + b)$ atau $ax^2 + bx + c$, maka koefisien hasil bagi dari skema Horner **wajib dibagi dengan $a$**. Sisa pembagian tidak perlu dibagi.

**2. Teorema Sisa (Pembagi Linear & Kuadrat):**
* Dibagi $(x - k) \implies S = P(k)$
* Dibagi kuadrat $(x - a)(x - b) \implies$ Sisa selalu berbentuk linear $S(x) = px + q$.

**3. Rumus Sakti Vieta (Pangkat 3):**
Untuk $ax^3 + bx^2 + cx + d = 0$:
* $x_1 + x_2 + x_3 = -\frac{b}{a}$
* $x_1x_2 + x_1x_3 + x_2x_3 = \frac{c}{a}$
* $x_1 x_2 x_3 = -\frac{d}{a}$

---
*Kembali ke Direktori Utama:* [[index teaching|🍎 Teaching Resources Hub]]
