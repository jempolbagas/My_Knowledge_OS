...
def _infer_topic(path: str) -> str:
    ...
    # e.g. 10 Spaces/11 College/Sem_4_Spring_2026/<TOPIC>/...
    # or   10 Spaces/12 Teaching/30 Sources/<TOPIC>/...
...
def build_summary():
    ...
    cursor.execute("SELECT path, title FROM nodes WHERE path LIKE '10 Spaces%'")
    ...
    # ── 00 Inbox ──────────────────────────────────────────────────────────
    cursor.execute("SELECT title FROM nodes WHERE path LIKE '00 Inbox%'")
